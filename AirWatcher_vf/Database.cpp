...
bool Database::updatePrivateIndivReliability(string UserId, bool isreliable){
    bool updated;
    for (long unsigned int i=0; i<privateIndivs.size(); i++)
    {
        if (privateIndivs[i].GetUserID() == UserId) 
        {
            privateIndivs[i].setReliability(isreliable);
            updated = true;
            break; 
        }
    }
    for (long unsigned int i=0; i<privateIndivs.size(); i++){
        privateIndivs[i].AfficherPrivateIndividual();
        cout<<endl;
    }
    if (!updated)
        cerr << "could not update the reliability for this private individual ID" <<endl;
    return updated;
}
